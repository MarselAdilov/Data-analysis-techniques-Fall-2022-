...
birthwt <- birthwt %>%
  filter(birthwt$mother.age <= 40) # убираем выплести
...
t.test (birthwt$birthwt.grams[birthwt$mother.smokes == "Yes"],
        birthwt$birthwt.grams[birthwt$mother.smokes == "No"]) # p-value should be < 0.05

linear.model.1 <- lm (birthwt.grams ~ mother.smokes, data=birthwt) # зависимость веса от курения
linear.model.1
summary(linear.model.1)

linear.model.2 <- lm (birthwt.grams ~ mother.age, data=birthwt) # зависимость веса от возраста
linear.model.2
summary(linear.model.1)
